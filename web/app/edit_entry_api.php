...
if ($method === 'GET') {
    ...
    try {
        ...
        $entry['ugyminoseg'] = $entry['subject'] ?? '';
    ...
    $rooms            = trim($_POST['room_number'] ?? '');
    $sorszam          = trim($_POST['sorszam'] ?? '');
    $start_input      = trim($_POST['kezd_ido'] ?? '');
    $end_input_raw    = trim($_POST['befejez_ido'] ?? '');
    $ugyszam          = trim($_POST['ugyszam'] ?? '');
    $persons          = trim($_POST['resztvevok'] ?? '');
    $letszam          = trim($_POST['letszam'] ?? '');
    ...
    if (!$start_ts || !$end_ts) {
        respond(['success' => false, 'message' => 'Érvénytelen időformátum (HH:MM).', 'data' => null], 422);
    }
    if ($start_ts >= $end_ts) {
